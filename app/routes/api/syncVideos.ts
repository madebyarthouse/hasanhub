...
export async function loader({ params }) {
  ...
  console.log(`# of Videos to be synced: ${videos.length}`);
  ...
  console.log(`# of Videos found: ${videosData.length}`);
    ...
    videosData.map((videoData, index) => {
      return prisma.video.update({
        ...
        data: {
          ...
          duration: toSeconds(parse(videoData.contentDetails.duration)) ?? null,
  ...
  console.log(`# of Videos updated: ${updated.length}`);

  prisma.$disconnect();

  return json(updated.map((video) => video.title));
}
