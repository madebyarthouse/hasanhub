...
import type {SearchVideosResponseType, YTChannelSnippet, YTPlaylistSnippet, YTVideoItem} from "./_youtube.server"
...
const ytChannelDTO = (ytData: YTChannelSnippet) => {
  ...
  const getLatestVideosForChannel = async (channelId: number): Promise<string[]> => {
  ...
  const videosContainLatestVideos = (
    videos: YTVideoItem[],
    latestVideos: string[],
  ) => {
  ...
  const fetchNewVideos = async (channel: Channel) => {
    const latestVideos = await getLatestVideosForChannel(channel.id);
    ...
    do {
      ...
      !videosContainLatestVideos(videos, latestVideos)
    );
  ...
  export default async function handler(request: VercelRequest, response: VercelResponse) {
    ...
    try {
      ...
      const data: { newVideos: ReturnType<typeof ytVideoDTO>[]; channel: Channel }[] = [];
      for (const channel of channels) {
        await processChannel(channel);
  
        const videosData = (await fetchNewVideos(channel)).map(
          (video) => ytVideoDTO(video, channel),
        );
